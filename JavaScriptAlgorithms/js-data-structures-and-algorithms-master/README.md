...
[./Chapter10(Search).js 설명](<./Chapter10(Search).js>)

> 선형 검색: 정렬된 자료와 정렬되지 않은 자료 모두에 사용 가능하기 때문에 매우 유연하다.

- 이진 검색에 비해 시간 복잡도가 높다.
- 배열의 모든 항목을 확인한다.

> 이진 검색: 정렬된 자료에 대해 사용한다.

- 중간 값을 확인해 원하는 값보다 해당 중간 값이 큰지 작은지 확인한다.

### 정렬

[./Chapter10(Sort).js 설명](<./Chapter10(Sort).js>)

> **거품 정렬(bubble sort)**: 전체 배열을 순회하면서 항목이 다른 항목보다 큰 경우 두 항목을 교환한다.  
> **선택 정렬(selection sort)**: 가장 작은 항목을 찾아서 해당 항목을 배열의 현 위치에 삽입한다.  
> **삽입 정렬(insertion sort)**: 배열을 순차적으로 검색하면서 정렬되지 않은 항목들을 배열의 왼쪽의 정렬된 부분으로 이동시킨다.  
> **빠른 정렬(quick sort)**: 기준점을 획득한 다음 해당 기준점을 기준으로 배열을 나눈다.(평균 성능 최적화돼야하는 경우 사용)  
> **빠른 선택(quick select)**: 정렬되지 않은 목록에서 k번째로 작은 항목을 찾는다. 빠른 정렬처럼 기준점의 양쪽 모두를 재귀적으로 수행하는 대신 하쪽만을 재귀적으로 수행한다.  
> **병합 정렬(merge sort)**: 각 하위 배열에 하나의 항목이 존재할 때까지 배열을 하위 배열로 나눈다. 그러고 나서 각 하위 배열을 정렬된 순서로 병합한다.  
> **계수 정렬(count sort)**: 값들을 비교하지 않기 때문에 O(k+n)시간 안에 수행된다. 숫자에 대해서만 동작하며 특정 범위가 주어져야 한다. 항목들을 교환하면서 정렬하는 대신에 배열의 각 항목의 등장 횟수를 센다. 각 항목의 등장 횟수를 센 다음 해당 등장 횟수를 사용해 새로운 배열을 생성할 수 있다. (제한된 범위의 정수를 정렬할 때 사용)  
> **자바스크립트 내장 정렬**: sort()
